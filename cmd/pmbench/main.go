...
func main() {
	for _, algo := range []struct{
		name string
		impl scanhash.ScanHash
		template string
		target string
	}{
		{
			name: "MD5",
			impl: scanhash.ScanHash{Hash: md5.New()},
			template: "hase   ",
			target: "8743b52063cd84097a65d1633f5c74f5",
		},
		{
			name: "SHA1",
			impl: scanhash.ScanHash{Hash: sha1.New()},
			template: "hase   ",
			target: "b89eaac7e61417341b710b727768294d0e6a277b",
		},
		{
			name: "SHA256",
			impl: scanhash.ScanHash{Hash: sha256.New()},
			template: "hase   ",
			target: "127e6fbfe24a750e72930c220a8e138275656b8e5d8f48a98c3c92df2caba935",
		},
		{
			name: "SHA512",
			impl: scanhash.ScanHash{Hash: sha512.New()},
			template: "hase   ",
		...
		stop := algo.impl.MeteredScan(algo.template, algo.target, out)
		res := <-out
